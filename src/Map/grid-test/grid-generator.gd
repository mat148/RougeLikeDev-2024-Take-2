...
@export var map_width: int = 310
@export var map_height: int = 310
...
func generate_dungeon(player: Entity) -> MapDataGrid:
	...
	var calc_width = map_width/(plot_size + road_size)
	var calc_height = map_height/(plot_size + road_size)
	
	var x: int = 0
	var y: int = 0
	for width in calc_width:
		for height in calc_height:
			var new_room := Rect2i(
		...
		#x += (plot_size + road_size)
		#y += (plot_size + road_size)
...
func _carve_tile(dungeon: MapDataGrid, x: int, y: int) -> void:
	var tile_position = Vector2i(x, y)
	var tile: TileGrid = dungeon.get_tile(tile_position)
	tile.set_tile_type(dungeon.tile_types.floor)

func _carve_room(dungeon: MapDataGrid, room: Rect2i) -> void:
